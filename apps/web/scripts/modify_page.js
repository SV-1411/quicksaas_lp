...
function WaitlistSection() {
  return (
    <section className="relative z-30 bg-[#080705] text-[#FAF9F6] py-[25vh] px-6 md:px-20 border-t border-[#080705]/8 overflow-hidden flex flex-col items-center justify-center text-center">
      ...
      <div className="relative z-10 max-w-4xl">
        <h2 className="text-[clamp(36px,5.5vw,80px)] font-light tracking-tighter leading-[0.9] mb-10">
          The future is <span className="text-primary italic font-serif">exclusive.</span>
        </h2>
        <p className="text-[#FAF9F6]/50 text-lg md:text-xl font-light tracking-wide mb-16 max-w-2xl mx-auto">
          Join the waitlist to get early access to our managed digital factory. Abstract operations, guarantee throughput.
        </p>
        <div className="flex flex-col md:flex-row items-stretch justify-center gap-4 max-w-xl mx-auto">
          <input 
            type="email" 
            placeholder="ENTER YOUR PROTOCOL ID (EMAIL)" 
...
// We want to replace from {/* ════ 2. THE CONCEPT ═══════════════════════════════════ */} onwards
const conceptIndex = content.indexOf('{/* ════ 2. THE CONCEPT ═══════════════════════════════════ */}');
const closeMainIndex = content.lastIndexOf('</main>');

if (conceptIndex !== -1 && closeMainIndex !== -1) {
...
`;
  ...
  content = content.substring(0, conceptIndex) + replacementTail + content.substring(closeMainIndex + 7);
}
